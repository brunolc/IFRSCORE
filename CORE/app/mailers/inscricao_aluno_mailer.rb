...
class InscricaoAlunoMailer < ActionMailer::Base
  ...
  def novaVaga(v, a)
    ...
    mail to: @aluno.email, subject: "(IFRSCORE) Uma empresa abriu uma nova vaga para estagiario"
  end

  def cadastro(a)
    @aluno = a
    mail to: @aluno.email, subject: "(IFRSCORE) E necessario confirmar seu email"
  end
end
